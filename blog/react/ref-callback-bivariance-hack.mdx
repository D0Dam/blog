...
```tsx
mouseEnter === true ? <EnterStarRateContainer rating={rating} /> : <LeaveStarRateContainer rating={rating} />;
```
...
function EnterStarRateContainer({ rating }: { rating: number }) {
	const { leave, click, active } = useContext(StarRateContext);
	const [starNum, setStarNum] = useState(rating); //별 순번
...
마지막에 코드를 기술하겠지만, `StarRatingContext`에서 꺼내 쓰고 있는 `leave`, `click`, `setActive`는 정말 말 그대로의 역할을 수행하는 함수입니다.
그래서 굳이 안의 함수가 어떤 구조인지 알지 않아도 코드를 이해하기 충분했습니다.
각 속성 이벤트에 들어간 함수들도 정말 깔끔합니다!
대표적으로 하나의 이벤트만 살펴보겠습니다.
`onMouseLeave`에서는 말 그대로 마우스가 떠났다는 `mouseLeave`와 별 순번 상태 초기화를 위한 `setStarNum(0)`이 들어가 있습니다.
...
function EnterStarRateContainer({ rating }: { rating: number }) {
	const { leave, click, setActive } = useContext(StarRateContext);
...
어떠신가요?
코드 길이가 거의 2배 이상 많지만 많은 사람들은 오히려 이쪽 손을 들어줄 것이라 생각합니다.
